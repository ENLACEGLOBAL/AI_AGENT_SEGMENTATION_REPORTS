...
class CrucesAnalyticsService:
    ...
    def _load_formularios_from_db(
        ...
        try:
            ...
            print(f"   ✅ Formularios cargados desde BD (tabla 'formularios'): {len(df)} registros")
            return df

        except Exception as e:
            print(f"   ⚠️ Error cargando tabla 'formularios': {e}")
            return pd.DataFrame(columns=['id_empresa', 'numero_id', 'fecha_registro', 'nombre_completo'])
        finally:
            local_forms.close()
        ...
        try:
            ...
            if df_forms is not None and not df_forms.empty:
                for _, r in df_forms.iterrows():
                    ...
                    nid = self.normalize_id(r.get("numero_id"))
                    if eid and nid:
                        forms_set.add((eid, nid))
                        # Si tiene longitud razonable, guardamos sin el DV por si acaso
                        if len(nid) > 5:
                            base = nid[:-1]
                            if base:
            ...
            for (_, _), ent in entidades_sin_dd_map.items():
                entidades_sin_dd.append({
                    ...
                    "proveedor": {"count": len(ent["proveedor_txs"]), "amount": ent["proveedor_sum"],
                                  ...
                                  "transacciones": ent["proveedor_txs"]},
                    "empleado": {"count": len(ent["empleado_txs"]), "amount": ent["empleado_sum"],
                                 "risk_class": "secondary", "risk_label": "N/A", "transacciones": ent["empleado_txs"]},
                    "risk_factors": {},
                    "riesgo_maximo": 0,
                    "dd": False,
            ...
            try:
                def extract_sid(entry: Dict[str, Any]) -> str:
                    # AGREGAMOS "id" A LA LISTA DE PRIMERO
                ...
                # ASEGURAR QUE empresa_id SEA UN ENTERO PARA EL MATCH
                ...
                if isinstance(tabla_detalles, list):
                    for e in tabla_detalles:
                        sid = extract_sid(e)
                        if sid:
                            if (empresa_id_safe, sid) in forms_set or (
